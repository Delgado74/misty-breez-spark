...
final Logger _breezSdkSparkLogger = Logger('BreezSdkSpark');
...
class BreezLogger {
  ...
  void _createSessionLogFile() async {
    try {
      final Directory appDir = await _getAppDir();
  ...
  void registerBreezSdkSparkLogs(BreezSDKSpark breezSdkSpark) {
    breezSdkSpark.logStream.listen(
      (spark_sdk.LogEntry e) => _logBreezSdkSparkEntries(e, _breezSdkSparkLogger),
    );
  }

  void _logBreezSdkSparkEntries(spark_sdk.LogEntry log, Logger logger) {
    switch (log.level) {
      case 'ERROR':
