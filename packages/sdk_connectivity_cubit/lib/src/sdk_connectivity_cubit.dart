...
class SdkConnectivityCubit extends Cubit<SdkConnectivityState> {
  ...
  SdkConnectivityCubit({required this.credentialsManager, required this.breezSdkSpark})
  ...
  Future<void> _connect(String mnemonic, {bool storeMnemonic = false}) async {
    try {
      ...
      _logger.warning('Failed to connect to Breez Spark SDK. Reason: $e');
      if (storeMnemonic) {
  ...
  void _startSyncing() {
    final SyncManager syncManager = SyncManager(breezSdkSpark.sdk);
